...
const ProductActions: React.FC<ProductActionProps> = ({ product }) => {
  ...
  const handleWhatsAppClick = () => {
    const message = `Halo kak, saya mau order ${product.name} dengan jumlah ${quantity} pcs. Total harga: Rp ${(product.price * quantity).toLocaleString()}. Mohon dibantu proses pesanannya ya. Terima kasih!`;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="add-to-cart -dark flex items-center space-x-2">
        ...
        <h5 className="font-medium" onClick={handleWhatsAppClick} >Beli Sekarang</h5>
      </div>
    </div>
