"use strict";
import { globalSliderSettings } from '../../src/components/elements/sliderSettings/sliderSettings';
...
function runHeroSliderAnimation() {
  function animationHandle(currentSlide) {
    // Validasi jika currentSlide tidak valid
    if (!currentSlide || currentSlide.length === 0) {
      console.warn("currentSlide is null or empty. Skipping animation.");
      return;
    }

    // Temukan elemen dengan atribut data-animation-in
    ...
    $.each(animationItems, function (index, item) {
      ...
      let animationDelay = $(item).data("animation-delay") || 0; // Default delay 0 jika undefined
      let addedAnimationName = `animate__${animationName}`;
  ...
  // Pastikan slider ada sebelum mencoba mengaksesnya
  if ($slider.length === 0) {
    console.error("Slider element not found. Please check the DOM structure.");
    return;
  }

  // Event init: Jalankan animasi pada slide aktif pertama
  $slider.on("init", function (e, slick) {
    let $currentSlide = $(slick.$slider).find(".slider__carousel__item.slick-active");
    animationHandle($currentSlide);
  });

  // Event beforeChange: Jalankan animasi pada slide berikutnya
  ...
  $slider.slick({
    ...
    fade: true,
    infinite: true,
    // Tambahkan pengaturan global jika ada
